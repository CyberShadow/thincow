...
# First block
( seq 1 1e99 || true ) | dd iflag=fullblock of=target/test bs=$block_size count=1 status=none conv=notrunc
dd if=/dev/zero                             of=target/test bs=$block_size count=1 status=none conv=notrunc

# Second block
( seq 2 1e99 || true ) | dd iflag=fullblock of=target/test bs=$block_size count=1 status=none conv=notrunc seek=1
